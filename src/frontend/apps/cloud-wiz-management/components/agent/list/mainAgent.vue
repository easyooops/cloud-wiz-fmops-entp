        ...
        <div class="tab-content" id="top-tabContent">  
          <div v-for="(item, index) in tab" :key="index" :class="{ 'tab-pane': true, 'fade': !item.active, 'active show': item.active }" :id="item.id" role="tabpanel" :aria-labelledby="item.label">  
            <div v-if="noData" class="no-data">  
              <vue-feather type="alert-circle" class="no-data-icon"></vue-feather>  
            ...
            <div v-else class="row">  
              <div class="col-lg-4 col-md-6" v-for="(dataItem, dataIndex) in filteredData" :key="dataIndex">  
                <div class="project-box" @click="navigateToEdit(dataItem.agent_id)" @mouseover="onMouseOver" @mouseleave="onMouseLeave">  
                  <span class="badge" :class="getBadgeClass('T', dataItem.embedding_enabled)">{{ getBadgeLabel(dataItem.fm_provider_type, dataItem.embedding_enabled) }}</span>  
...
export default {  
  ...
  computed: {  
    ...
    noData() {  
      return !this.loading && this.filteredData.length === 0;  
    }  
  },  
  methods: {  
    ...
    navigateToEdit(id) {  
      this.$router.push({ path: '/agent/create', query: { agentId: id } });  
    },  
    onMouseOver(event) {  
